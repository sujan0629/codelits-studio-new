      ...
      >
        <Card className="relative flex flex-col overflow-hidden transition-all duration-300 bg-card border-2 border-border/50 h-full">
          <CardContent className="p-0 relative aspect-[4/3]">
             <div className={cn(
                "absolute inset-0 bg-muted flex flex-col items-center justify-center transition-opacity duration-500",
                (isLoaded && iframeSrc) ? 'opacity-0 pointer-events-none' : 'opacity-100'
             )}>
                {!isHovered && <p className="text-muted-foreground text-sm px-4 text-center">Hover to load live preview</p>}
                {isHovered && !isLoaded && iframeSrc && (
                    <AtomLoader />
                )}
             </div>
             {iframeSrc && (
