...
export function Header() {
  ...
  // Control dropdown open states for hover
  ...
  // Close all dropdowns on mouse leave from nav container
  ...
  return (
    <header ref={headerRef} className="sticky top-0 z-50 w-full p-2 md:p-4 bg-white">
        ...
        >
          <NavigationMenu className="flex gap-4">
            <NavigationMenuList className="flex gap-4 text-sm font-medium items-center">
              ...
              >
                {/* Trigger with Link to /portfolio */}
                <Link
                  href="/portfolio"
                  className="nav-click-anim"
                  onClick={() => setWorkOpen(false)}
                >
                  <span className="cursor-pointer select-none px-2 py-1 rounded hover:bg-gray-100">
                    Work
                  </span>
                </Link>
                {workOpen && (
                  <NavigationMenuContent>
                    <ul className="grid w-[600px] p-4 grid-cols-2 gap-3 bg-white shadow-lg rounded-md">
                      {workItems.map((item) => (
