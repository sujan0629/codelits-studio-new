...
const aboutComponents = [
  { title: "About Us", href: "/team", description: "Meet the passionate creators, thinkers, and innovators.", icon: Users },
  { title: "Our Process", href: "/process", description: "How we turn great ideas into exceptional products.", icon: FileText },
  { title: "Blog", href: "/blog", description: "Read our latest articles, insights, and news.", icon: PencilLine },
];
              ...
              <NavigationMenuContent>
                <ul className="grid gap-3 p-4 w-[600px] lg:grid-cols-[.75fr_1fr]">
                  ...
                  {aboutComponents.map((item) => (
                    <ListItem
                      key={item.title}
                      title={<div className="flex items-center gap-2"><item.icon className="h-4 w-4" />{item.title}</div>}
                      href={item.href}
                    >
                      {item.description}
                    </ListItem>
