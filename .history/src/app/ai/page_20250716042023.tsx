...
const AiBrainAnimation = () => {
  ...
  useEffect(() => {
    ...
    gsap.to(wheelRef.current, {
      rotation: 360,
      transformOrigin: '100px 100px', // center of the SVG
      duration: 10, // spin duration in seconds
      repeat: -1,
      ease: 'linear',
    });
  }, []);
  ...
  return (
    <svg viewBox="0 0 200 200" className="w-full h-full max-w-md mx-auto">
      ...
      {/* This group contains the lines and endpoint circles that spin around center */}
      <g ref={wheelRef} opacity="0.2" className="animate-pulse">
        {/* Lines (spokes) */}
        <path d="M 100 100 L 50 50" stroke="hsl(var(--primary))" strokeWidth="0.5" />
        <path d="M 100 100 L 150 50" stroke="hsl(var(--primary))" strokeWidth="0.5" />
        <path d="M 100 100 L 50 150" stroke="hsl(var(--primary))" strokeWidth="0.5" />
        <path d="M 100 100 L 150 150" stroke="hsl(var(--primary))" strokeWidth="0.5" />
        <path d="M 100 100 L 30 100" stroke="hsl(var(--primary))" strokeWidth="0.5" />
        <path d="M 100 100 L 170 100" stroke="hsl(var(--primary))" strokeWidth="0.5" />
        <path d="M 100 100 L 100 30" stroke="hsl(var(--primary))" strokeWidth="0.5" />
        <path d="M 100 100 L 100 170" stroke="hsl(var(--primary))" strokeWidth="0.5" />

        {/* Endpoint nodes that spin with lines */}
        <circle cx="50" cy="50" r="8" fill="hsl(var(--primary) / 0.8)" />
        <circle cx="150" cy="50" r="8" fill="hsl(var(--primary) / 0.8)" />
        <circle cx="50" cy="150" r="8" fill="hsl(var(--primary) / 0.8)" />
        <circle cx="150" cy="150" r="8" fill="hsl(var(--primary) / 0.8)" />
        <circle cx="30" cy="100" r="6" fill="hsl(var(--primary) / 0.6)" />
        <circle cx="170" cy="100" r="6" fill="hsl(var(--primary) / 0.6)" />
        <circle cx="100" cy="30" r="6" fill="hsl(var(--primary) / 0.6)" />
        <circle cx="100" cy="170" r="6" fill="hsl(var(--primary) / 0.6)" />
      </g>

      {/* The center node stays fixed */}
    ...
    <div className="relative overflow-hidden bg-dark text-foreground min-h-screen ">
      ...
      <section className="py-20 md:py-28 max-w-3xl mx-auto px-4 text-center relative ">
        ...
        <div className="relative w-full max-w-md mx-auto aspect-square">
          ...
          {/* Step 1: Top */}
          <div
            className="absolute top-2 left-1/2 transform -translate-x-1/2 bg-transparent bg-opacity-10 border border-primary border-opacity-30 rounded-md px-6 py-3 flex items-center gap-2 w-48 mx-auto"
...
{/* Step 3: Bottom right */}
<div
  className="absolute bottom-10 right-[100%] bg-transparent bg-opacity-10 border border-white border-opacity-30 rounded-md px-6 py-3 flex items-center gap-2 w-48"
  style={{ backdropFilter: 'blur(8px)' }}
>
  <Check className="w-6 h-6 text-primary" />
