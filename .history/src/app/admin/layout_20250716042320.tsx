...
import { Bot, Image, Pilcrow, Search, Settings, User, ArrowLeft, Menu, X } from 'lucide-react';
  ...
  return (
    <TooltipProvider>
      <div className="flex min-h-screen w-full bg-muted/40">
        ...
        <aside
          className={cn(
            'fixed inset-y-0 left-0 z-40 w-28 border-r bg-background sm:static sm:h-screen sm:flex sm:flex-col sm:translate-x-0 transition-transform duration-300 ease-in-out',
            {
              '-translate-x-full': !sidebarOpen,
              'translate-x-0': sidebarOpen,
              'hidden sm:flex': true, // hide on mobile but show on sm+
            }
          )}
        ...
        >
          ...
          <div className="flex items-center justify-center border-b px-4 py-3">
            ...
            <button
              className="ml-auto inline-flex items-center justify-center rounded-md p-2 text-muted-foreground hover:bg-muted/30 sm:hidden"
