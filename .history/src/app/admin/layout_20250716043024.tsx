...
<aside
  className={cn(
    'fixed inset-y-0 z-40 border-r bg-background sm:static sm:h-screen sm:flex sm:flex-col transition-transform duration-300 ease-in-out',
    {
      'w-80 left-1/2 -translate-x-1/2': !sidebarOpen, // centered off-screen for closed state
      'w-80 left-1/2 -translate-x-1/2 translate-x-0': sidebarOpen, // centered visible for open state
      'sm:left-0 sm:translate-x-0 sm:w-28 sm:flex': true, // desktop aligned left, normal width
    }
  )}
...
<header className="z-50 flex h-14 items-center justify-between border-b bg-background px-4 sm:px-6 sm:h-auto sm:border-0 sm:bg-transparent sm:py-4">
            <div className="flex items-center gap-3">
              {/* Hamburger menu for mobile */}
              <button
                className="sm:hidden flex items-center justify-center rounded-md p-2 text-muted-foreground hover:bg-muted/30"
