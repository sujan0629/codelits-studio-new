      ...
      <div className="mx-auto max-w-5xl px-6 space-y-12">
        ...
        <div className="relative">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            {features.slice(0, 2).map(({ title, description, imgSrc, imgAlt, Icon, align }, i) => (
              <div
                key={i}
                className={`feature space-y-4 text-left ${align === 'right' ? 'md:text-right' : ''}`}
              >
                <h3 className="text-2xl font-bold flex items-center gap-2 justify-start md:justify-end">
                  {Icon && <span>{Icon}</span>}
                  <span>{title}</span>
                </h3>
