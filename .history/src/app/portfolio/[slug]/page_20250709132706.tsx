  ...
  return (
    <div className="flex flex-col">
      <div className="w-full px-4 py-6 md:px-8 md:py-8 border-b bg-background sticky top-0 z-20 backdrop-blur-md">
        <Link href="/portfolio" className="inline-flex items-center text-sm text-muted-foreground hover:text-primary mb-2">
          <ArrowLeft className="mr-2 h-4 w-4" />
          Back to Portfolio
        </Link>
        ...
        <div className="relative w-full flex-grow overflow-hidden">
          {canBeIframed ? (
            <>
              {!isLoaded && (
                <div className="absolute inset-0 flex items-center justify-center z-10 bg-background">
                  ...
                  <span className="text-sm text-muted-foreground">Loading Live Project...</span>
                </div>
              )}
              <iframe
                src={project.link}
                title={project.title}
                className={cn("w-full h-[80vh] border-0", isLoaded ? 'opacity-100' : 'opacity-0')}
            ...
            <div className="w-full h-[80vh] flex items-center justify-center text-center">
              ...
              <p className="text-muted-foreground">Live preview not available.</p>
            </div>
          )}
        </div>
      </div>
