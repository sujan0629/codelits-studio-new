...
>
  {allProjects.map((project) => (
    <div
      key={project.slug}
      className="project-wrapper w-[95vw] max-w-7xl flex items-start space-x-10"
    >
      {/* LEFT: larger image box */}
      <div className="iframe-box w-2/3 h-[500px] bg-card border border-border rounded-2xl overflow-hidden relative pointer-events-none select-none">
        <img
          src={project.image}
          alt={project.title}
      ...
      <div className="project-details flex-1 flex flex-col justify-center">
        ...
        <div className="flex flex-wrap gap-2 mb-3">
          {project.tags.map((tag) => (
            <Badge key={tag} variant="secondary" className="text-sm px-2 py-1">{tag}</Badge>
          ))}
        </div>
        <p className="text-muted-foreground text-sm md:text-base mb-5">
          {project.longDescription}
        </p>
        <Button asChild variant="outline" size="sm" className="text-sm px-4 py-2">
