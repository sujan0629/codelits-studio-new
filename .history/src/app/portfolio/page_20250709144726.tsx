...
export default function PortfolioShowcase() {
  ...
  useEffect(() => {
    ...
    // Setup initial state: all projects hidden except first
    gsap.set(wrappers, {
      position: 'absolute',
      top: 0,
      left: 0,
    ...
    // Reset all image boxes rotation/position
    wrappers.forEach(wrapper => {
      const imageBox = wrapper.querySelector('.image-box') as HTMLElement;
      if (imageBox) {
        gsap.set(imageBox, {
          rotationX: 0,
          rotationY: 0,
    ...
    wrappers.forEach((wrapper, i) => {
      ScrollTrigger.create({
        ...
        onEnter: () => {
          ...
          // Animate image box with 3D move/rotate effect
          const imageBox = wrapper.querySelector('.image-box') as HTMLElement;
          if (imageBox) {
            gsap.fromTo(
              imageBox,
        ...
        onLeave: () => {
          // Hide current project on leave
          gsap.set(wrapper, { autoAlpha: 0 });
          // Reset image box transform to zero instantly
          const imageBox = wrapper.querySelector('.image-box') as HTMLElement;
          if (imageBox) {
            gsap.set(imageBox, { rotationX: 0, rotationY: 0, x: 0, y: 0 });
          }
        },
        onLeaveBack: () => {
          gsap.set(wrapper, { autoAlpha: 0 });
    ...
    <>
      ...
      >
        {allProjects.map((project) => (
          ...
          >
            {/* Image box with class for targeting */}
            <div
              className="image-box w-full md:w-2/3 h-[500px] bg-card border border-border rounded-2xl overflow-hidden relative select-none"
