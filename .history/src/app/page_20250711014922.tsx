...
function ProductDirection() {
  ...
  useEffect(() => {
    const ctx = gsap.context(() => {
      ...
      tl.fromTo(".direction-label", { y: 20, opacity: 0 }, { y: 0, opacity: 1, duration: 0.8 })
        .fromTo(".direction-title", { y: 50, opacity: 0 }, { y: 0, opacity: 1 }, "-=0.6")
        .fromTo(".direction-p", { y: 20, opacity: 0 }, { y: 0, opacity: 1 }, "-=0.8")
        .fromTo(".direction-image-container", { y: 100, opacity: 0, scale: 0.9, rotationX: -40, rotationZ: -10 }, { y: 0, opacity: 1, scale: 1, rotationX: -25, rotationZ: -5, duration: 1.5, ease: 'elastic.out(1, 0.75)' }, "-=1.2");
  ...
  return (
    <section ref={sectionRef} className="relative py-20 md:py-32 overflow-hidden bg-background">
      <div className="container mx-auto px-4 z-10 relative text-center">
        ...
        <h2 className="direction-title font-headline text-3xl md:text-5xl font-bold tracking-tighter mt-6">
          Set the product direction
        </h2>
        <p className="direction-p mt-6 max-w-2xl mx-auto text-lg md:text-xl text-muted-foreground">
          Align your team around a unified product timeline. Plan, manage, and track all product initiatives with CodeLits's visual planning tools.
        </p>
      </div>

      <div className="direction-image-container mt-20 md:mt-24 perspective-1000 relative max-w-6xl mx-auto">
        <div
          className={cn(
            "relative aspect-[12/6] rounded-2xl overflow-hidden shadow-2xl",
            "[mask-image:radial-gradient(ellipse_100%_40%_at_50%_60%,black,transparent)]",
            "transform-gpu",
            "[transform:rotateX(-25deg)_rotateZ(-5deg)]"
          )}
          style={{
            filter: 'drop-shadow(0 0 15px transparent)', // remove glow by making shadow transparent
          }}
        >
          {/* Add side blurs */}
          <div
            aria-hidden="true"
            className="pointer-events-none absolute top-0 left-0 h-full w-12 bg-gradient-to-r from-black/80 to-transparent"
            style={{ filter: 'blur(12px)' }}
          />
          <div
            aria-hidden="true"
            className="pointer-events-none absolute top-0 right-0 h-full w-12 bg-gradient-to-l from-black/80 to-transparent"
            style={{ filter: 'blur(12px)' }}
          />
          <Image
            ...
            data-ai-hint="product timeline gantt"
          />
        </div>
      </div>
    </section>
